...
- 何を解くか: 障害初動で共有情報が散らばり、判断が遅れる問題を減らす。
- 何を実装したか: 障害原文を保存し、AIで要点を構造化して返す API を実装した。
- 学生としての強み: 速報性だけでなく、監査性と再現性を両立するデータ設計を意識している。

## 面接での60秒説明テンプレ

1. 課題: 障害時は情報量が多く、関係者への共有が遅れやすい。  
2. 設計: `IncidentReport` と `IncidentBriefing` を分離し、原文と要約を独立管理した。  
3. 技術選択: AI連携失敗時も原文が残るようにし、再試行可能なフローを採用した。  
4. 成果: オンコールの初動で「原因/影響/次アクション」を短時間で共有できる構造になった。
